...
def create_results_zip(images_data, zip_name):
    """
    Create an in-memory ZIP file from the processed images.
    images_data: dict with keys -> { 'filename': str, 'image': np.ndarray }
    """
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for key, info in images_data.items():
...
def index():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Base64 versions for inline display on the web page
            images_base64 = {
                key: {
                    "data": image_to_base64(info["image"]),
                    "filename": info["filename"],
                }
                for key, info in raw_images.items()
            }

            # Create ZIP archive containing all results
            zip_info = create_results_zip(raw_images, f"{base_name}_denoising_results.zip")

            # Return JSON response with images, stats, and zip archive
            return jsonify({
                "success": True,
